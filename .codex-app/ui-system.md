...
- On the terminal screen, keyboard open/close should use the route-scoped Android `adjustPan` path with `resizeToAvoidBottomInset: false`; avoid UI changes that force the native terminal and browser platform views to relayout during IME transitions. The terminal page shortcut bar is now native, not a Flutter overlay, and should ride with the terminal as a single surface above the IME. Native terminal keyboard open should avoid redundant `showSoftInput` retries once focus is already active, so the prompt still appears above the keyboard without the extra lag caused by duplicate initial requests.
