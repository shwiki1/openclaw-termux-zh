...
- Terminal screen IME layout policy: `TerminalScreen` must keep `resizeToAvoidBottomInset: false` and acquire Android `adjustPan` while the route is visible. Resizing the Flutter scaffold for keyboard insets forces the terminal `AndroidView` and browser `WebView` to relayout together and causes visible IME jank; use route-scoped soft-input panning so terminal prompts, the browser address bar, and WebView form inputs can stay above the keyboard without shrinking the whole Flutter page.
...
- Keep `TerminalScreen.resizeToAvoidBottomInset` disabled and preserve the route-scoped `adjustPan` soft-input handoff while the native terminal/browser platform-view layout shares the same page unless a replacement IME strategy is implemented and verified on-device.
