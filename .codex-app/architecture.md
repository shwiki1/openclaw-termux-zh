...
- Native/web/runtime boundary: Flutter calls `NativeBridge` (`flutter_app/lib/services/native_bridge.dart`) over `com.agent.cyx/native`; Kotlin returns logs through `com.agent.cyx/gateway_logs` and `com.agent.cyx/setup_logs`; terminal launch now has two native front ends: the legacy `NativeTerminalView` Flutter platform-view wrapper and the newer full-screen `NativeTerminalActivity`.
...
- Terminal IME layout policy: the ZeroTermux/Termux-smooth path is a dedicated native Android terminal screen, not a Flutter `AndroidView` page with more compensation. `CliToolsScreen` should therefore launch `NativeTerminalActivity` through `NativeBridge.openNativeTerminalActivity(...)` for CLI/Codex sessions, so IME show/hide resizes only the Android view tree that contains `TerminalView` and the native shortcut bar. `NativeTerminalSessionView.kt` must keep keyboard reopening lightweight: normal taps request a single native `showSoftInput(...)` without `restartInput(...)`, only fresh session startup/restart may keep one delayed retry, and pending keyboard retry callbacks must be cleared on hide/dispose so stale callbacks do not bleed into later terminal opens. Because the native terminal prompt is not a Flutter `TextField`, the shared session view must keep the bottom input-strip `requestRectangleOnScreen(...)` helper for non-toolbar cases, but the native terminal activity itself should rely on `adjustResize` and same-tree layout instead of reintroducing a Flutter-side IME compensation chain. Browser-side input inside `TerminalBrowserPanel` still uses `NativeBridge` soft-input arbitration and may temporarily switch the Flutter activity to `adjustNothing`, but that browser path is separate from the new native CLI terminal activity. This logic is an original rewrite inspired by official Termux and ZeroTermux behavior; do not paste GPLv3 source into this repo as a shortcut.
...
## Module Boundaries
- UI/screens: `flutter_app/lib/screens/*` render setup, dashboard, providers, config, local model, package, backup, logs, message platform, settings, and the legacy Flutter terminal/browser route. `CliToolsScreen` now launches the native Android terminal activity instead of pushing `TerminalScreen`.
...
- Native Android: `MainActivity.kt` registers channels and delegates to managers/services; `NativeTerminalActivity.kt` hosts full-screen terminal sessions; each foreground service owns its own lifecycle.
...
- CLI terminal: `CliToolsScreen` -> `TerminalService` config/args resolution -> `NativeBridge.openNativeTerminalActivity` -> `MainActivity` method channel -> `NativeTerminalActivity` -> shared `NativeTerminalSessionView` -> `TerminalSessionService` + RootFS shell process.
...
- CLI tools terminal sessions are now a native Android activity launch, not a Flutter route push.
...
- Keep `CliToolsScreen` launching terminal sessions through `NativeBridge.openNativeTerminalActivity(...)`, keep `NativeTerminalActivity` on `adjustResize`, and keep `NativeTerminalSessionView.kt` as the shared terminal core. Do not casually move the active CLI/Codex terminal path back onto a Flutter `AndroidView` route or restore the removed Flutter/global-layout IME compensation chain as the primary fix. Browser focus inside the compact or wide `TerminalBrowserPanel` must continue to use the shared `NativeBridge` soft-input arbitration so WebView form fields do not get double-lifted by both Android resize handling and WebView internal scrolling.
