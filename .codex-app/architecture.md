...
- Keep `TerminalScreen.resizeToAvoidBottomInset` disabled, preserve the route-scoped `adjustPan` soft-input handoff, and keep the native terminal bottom-focus-strip helper in `NativeTerminalView.kt`. For Codex terminal sessions, keep the shortcut bar in Flutter above the terminal and keep `useNativeToolbar: false` unless on-device evidence shows that design is worse. Reserve the native global-layout overlap compensation for native-toolbar sessions only; do not revive Codex-specific bottom compensation casually. Browser focus inside the compact or wide `TerminalBrowserPanel` must continue to use the shared `NativeBridge` soft-input arbitration so WebView form fields do not get double-lifted by both Android `adjustPan` and WebView internal scrolling.
