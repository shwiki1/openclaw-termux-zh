...
## Module Boundaries
- UI/screens: `flutter_app/lib/screens/*` render setup, dashboard, providers, config, local model, package, backup, logs, message platform, settings, and the legacy Flutter terminal/browser route. `CliToolsScreen` now launches native Android terminal surfaces: `NativeTerminalPagerActivity` for Codex and `NativeTerminalActivity` for other CLI tools.
