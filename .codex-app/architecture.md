...
- Terminal screen IME layout policy: `TerminalScreen` should allow Flutter to apply the real `viewInsets` for the keyboard (`resizeToAvoidBottomInset: true`) so the embedded Android terminal platform view actually receives the reduced height when the IME opens. Terminal ownership of the Android window soft-input mode should prefer `adjustResize`, not `adjustPan`. This matches the official Termux baseline more closely: the native terminal container shrinks with the IME, so the shortcut bar can sit above the keyboard as part of the same Android layout, while browser-focused inputs in `TerminalBrowserPanel` still temporarily switch the route to `adjustNothing`. The terminal page keeps one shortcut strategy for app terminal sessions: Codex and non-Codex both use the native platform-view toolbar (`useNativeToolbar: true`) inside `NativeTerminalView`, while the standalone Flutter `TerminalToolbar` remains only for setup/installer flows. Because the native terminal prompt is not a Flutter `TextField`, `NativeTerminalView.kt` must keep the bottom input-strip `requestRectangleOnScreen(...)` helper, but native-toolbar sessions should not issue an extra parent-container toolbar visibility request or a follow-up global-layout compensation chain on top of `adjustResize`; those extra requests over-pan the command area without actually fixing the missing toolbar lift. Keep terminal keyboard reopening lightweight: normal taps should request a single native `showSoftInput(...)` without `restartInput(...)`, only fresh session startup/restart may keep one delayed retry, and pending keyboard retry callbacks must be cleared on hide/dispose so stale platform-view callbacks do not bleed into later terminal opens. Browser-side input inside `TerminalBrowserPanel` must not reuse the terminal resize path once focus moves into the Flutter address bar or a WebView form field; `NativeBridge` arbitrates soft-input mode globally so browser focus temporarily switches the route to `adjustNothing`, then hands control back to terminal `adjustResize` after blur or panel hide. This logic is an original rewrite inspired by official Termux and ZeroTermux behavior; do not paste GPLv3 source into this repo as a shortcut.
...
- Keep the app terminal shortcut bar inside the native platform view for Codex and non-Codex sessions alike, keep `NativeBridge` terminal ownership on `adjustResize`, keep `TerminalScreen.resizeToAvoidBottomInset` enabled for the terminal route, and keep the native terminal bottom-focus-strip helper in `NativeTerminalView.kt`. Do not casually move Codex back to a Flutter overlay or restore the removed native IME compensation/global-layout chain. Browser focus inside the compact or wide `TerminalBrowserPanel` must continue to use the shared `NativeBridge` soft-input arbitration so WebView form fields do not get double-lifted by both Android resize handling and WebView internal scrolling.
