...
- Android install-visible version policy: manifest `versionName` is built as `base+logicalBuildNumber` while Flutter split-apk `versionCode` may be ABI-offset-adjusted; user-facing version displays should use `versionName` or `AppConstants.displayVersion`, not the raw split `versionCode`.
...
- Keep Android installer-visible version strings sourced from manifest `versionName` built as `base+build`, while update comparison continues to use the base semver plus numeric build separately.
