...
- Keep `TerminalScreen.resizeToAvoidBottomInset` disabled, preserve the route-scoped `adjustPan` soft-input handoff, and keep the native terminal bottom-focus-strip helper in `NativeTerminalView.kt`. Keep the app terminal shortcut bar inside the native platform view for Codex and non-Codex sessions alike; do not casually move Codex back to a Flutter overlay or restore the removed native IME compensation chain. Browser focus inside the compact or wide `TerminalBrowserPanel` must continue to use the shared `NativeBridge` soft-input arbitration so WebView form fields do not get double-lifted by both Android `adjustPan` and WebView internal scrolling.
