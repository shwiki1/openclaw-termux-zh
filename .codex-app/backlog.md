...
- Device-smoke the redesigned Codex terminal shortcut path on Android: top Flutter shortcut bar, repeated IME dismiss/reopen, immediate shortcut-key taps after IME close, and leave/re-enter terminal. If lag remains, focus on native `TerminalView`/`adjustPan` and platform-view/session reattach timing rather than reviving bottom compensation.
