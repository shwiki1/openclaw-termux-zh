...
## Ready
- Re-run `flutter analyze` and `flutter test` in CI or a Flutter SDK environment to verify the local `codex_config` test fix and triage any remaining analyzer reports.
- Device-smoke Codex browser automation from the terminal screen: first browser open shows the `Codex 浏览器自动化控制` instructions page, self-test, open URL, wait selector, scroll, type, press Enter, select option, capture snapshot, then close/reopen the compact right browser sidecar and verify it still shows `浏览器已连接`.
...
- Release signing is blocked until GitHub signing secrets (`KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`, `KEY_ALIAS`, `KEY_PASSWORD`) are configured.
...
- Current metadata is aligned at source `2.0.50+135`; the latest GitHub artifact was CI version `2.0.50+135` from run `29262431252`; the next cloud build should produce `2.0.50+136` or higher.
