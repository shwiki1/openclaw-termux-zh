...
从当前 2.0.x 分支开始，APK 不再假定内置完整大体积 RootFS / Node.js 运行时资源，以降低安装包体积。首次初始化时，应用会在线下载所需资源；如果网络较慢，也可以使用本 Release 的附件作为外部资源。

如果在 App 里使用，进入安装页的“预构建资源配置”，可以直接点击“使用 GitHub 资源”。应用会自动填入 GitHub RootFS/Ubuntu base 链接，并使用默认 Node.js 镜像链接：
...
https://npmmirror.com/mirrors/node/v24.15.0/node-v24.15.0-linux-arm64.tar.xz
```
...
| `node-v24.15.0-linux-arm64.tar.xz` | Node.js 24.15.0 arm64 运行时压缩包，供安装流程下载或本地缓存复用；如需作为 GitHub 附件使用，需同步上传该文件 | 28.71 MB |
...
- [node-v24.15.0-linux-arm64.tar.xz](https://npmmirror.com/mirrors/node/v24.15.0/node-v24.15.0-linux-arm64.tar.xz)
