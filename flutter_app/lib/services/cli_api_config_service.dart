...
class CliApiConfigService {
  ...
  static const _cliWorkspaceSkillPath =
      '$cliWorkspacePath/.agents/skills/ciyuanxia-android-runtime/SKILL.md';
  ...
  static Future<void> regenerateRuntimeFiles({
    ...
    await writeRootfsFile(
      _codexProxyEnvPath,
      '# Legacy Codex proxy is disabled. Use local api2py relay on 127.0.0.1:9999.\n',
    );
    await writeRootfsFile(
    ...
    await NativeBridge.runInProot(
      ...
      '"$cliWorkspacePath/.agents/skills/ciyuanxia-android-runtime" '
      '$_managedCliBinDir '
...
;
  ...
  static String _buildGlobalEnvFile() {
    return [
      '# Generated by CiYuanXia app. Safe to source from CLI wrappers.',
...
''';
  ...
  static String _buildToolEnvFile(String toolId, CliApiConfig config) {
    if (!_shouldManageToolRuntime(config)) {
      if (toolId == 'codex' && config.apiKey.trim().isNotEmpty) {
        return [
          '# Generated by CiYuanXia app. Codex uses the official OpenAI API endpoint.',
    ...
    final lines = <String>[
      '# Generated by CiYuanXia app. Contains user API configuration.',
      'export OPENCLAW_TOOL_ID=${_shQuote(toolId)}',
  ...
  static String _buildQwenSettingsJson(CliApiConfig config) {
    ...
    final modelEntry = <String, dynamic>{
      'id': modelId,
      'name': modelId,
      'description': 'CiYuanXia configured model',
      'envKey': envKey,
      ...
      'models': [
        {
          ...
          'description': 'CiYuanXia configured model - $baseUrl',
  ...
  static String _buildHermesConfigYaml(CliApiConfig config) {
    ...
    final model = config.effectiveToolModel.trim().isEmpty
        ...
        : config.effectiveToolModel.trim();
    final lines = <String>[
      '# Generated by CiYuanXia app. Hermes Agent runtime config.',
      'model:',
      '  provider: custom',
  ...
  static String _buildHermesEnvFile(CliApiConfig config) {
    ...
    final lines = <String>[
      '# Generated by CiYuanXia app. Hermes Agent environment.',
      if (config.apiKey.trim().isNotEmpty)
  ...
  static String _buildCodexToml(CliApiConfig codex) {
    ...
    lines.add('# Generated by CiYuanXia app. Safe to regenerate.');
...
const TOOL_DEFS = [
  {
    ...
    description:
      "Open the CiYuanXia in-app browser panel if needed and verify the bridge, WebView loading, and JavaScript execution using a local test page.",
    inputSchema: {
      type: "object",
      properties: {},
  ...
  },
  {
    name: "browser_open",
    description:
      "Open a URL in the CiYuanXia in-app browser panel. Use this before clicking or extracting page content.",
    inputSchema: {
      type: "object",
      properties: {
        url: {
  ...
  },
  {
    ...
    description:
      "List saved CiYuanXia browser automation scripts and the current pending-save draft, including filenames, descriptions, quick commands, and run metadata.",
    inputSchema: {
      type: "object",
      properties: {
        filter: {
...
async function callBridge(action, payload) {
  ...
  if (!baseUrl || !token) {
    throw new Error(
      "CiYuanXia browser bridge is not ready. Open the browser panel in the app first.",
    );
  }
...
if [ -z "\$base_url" ] || [ -z "\$token" ]; then
  echo "CiYuanXia browser bridge is not ready. Open the Codex browser panel first." >&2
  exit 1
fi
...
description: Control the CiYuanXia in-app browser panel from Codex CLI using deterministic browser tools.
---

Use this skill when the user asks you to inspect a webpage, click through a flow, fill a web form, or extract page content from CiYuanXia's in-app browser.
...
''';
  ...
  static String _buildCliWorkspaceGeminiMd() {
    return '''
# CiYuanXia Android Ubuntu Context
...
''';
  ...
  static String _geminiCustomModelAlias(CliApiConfig config) {
    ...
    final normalized = source
        ...
        .replaceAll(RegExp(r'^-+|-+$'), '');
    return normalized.isNotEmpty ? normalized : 'ciyuanxia';
  }

  static String _buildCliWorkspaceSkill() {
    return '''
---
name: ciyuanxia-android-runtime
