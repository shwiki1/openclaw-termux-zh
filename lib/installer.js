...
export function installOpenClaw() {
  ...
  try {
    execSync('npm install -g openclaw --registry=https://registry.npmmirror.com', { stdio: 'inherit' });
    return true;
  } catch (err) {
...
export function setupProotUbuntu() {
  ...
  const setupScript = `
    ...
    echo 'deb http://mirrors.tuna.tsinghua.edu.cn/ubuntu-ports noble main restricted universe multiverse' > /etc/apt/sources.list
    ...
    mkdir -p /tmp/openclaw-node && cd /tmp/openclaw-node
    curl -fL --retry 3 -o node.tar.xz https://npmmirror.com/mirrors/node/v24.15.0/node-v24.15.0-linux-arm64.tar.xz || curl -fL --retry 3 -o node.tar.xz https://mirrors.ustc.edu.cn/node/v24.15.0/node-v24.15.0-linux-arm64.tar.xz
    tar -xJf node.tar.xz
    cp -a node-v24.15.0-linux-arm64/{bin,include,lib,share} /usr/local/
