...
async function runTests() {
  ...
  test('terminal screen keeps IME from resizing the platform-view layout', () => {
    ...
    assert.ok(terminalScreen.includes('useNativeToolbar: !_isCodexSession'));
    assert.ok(terminalScreen.includes('if (_isCodexSession)\n          TerminalToolbar('));
  ...
  test('native terminal keeps IME show requests lightweight while preserving retry-on-startup handling', () => {
    ...
    assert.ok(nativeTerminalHost.includes('private val imeTransitionSettleRunnable = Runnable'));
    assert.ok(nativeTerminalHost.includes('private val imeTransitionFinishRunnable = Runnable'));
    assert.ok(nativeTerminalHost.includes('terminalView.removeCallbacks(keyboardRetryRunnable)'));
    assert.ok(nativeTerminalHost.includes('private var pendingImeCompensationBottomPx = 0'));
    assert.ok(nativeTerminalHost.includes('client.setImeTransitionActive(true)'));
    assert.ok(nativeTerminalHost.includes('applyPendingImeCompensation()'));
    assert.ok(nativeTerminalHost.includes('container.removeCallbacks(imeTransitionFinishRunnable)'));
    assert.ok(nativeTerminalHost.includes('IME_POST_LAYOUT_REFRESH_DELAY_MS = 32L'));
    assert.ok(nativeTerminalHost.includes('private fun applyPendingImeCompensation()'));
    assert.ok(nativeTerminalHost.includes('hiddenBottomPx == pendingImeCompensationBottomPx'));
    assert.ok(nativeTerminalHost.includes('IME_TRANSITION_SETTLE_DELAY_MS = 140L'));
    assert.ok(nativeTerminalHost.includes('private var imeTransitionActive = false'));
    assert.ok(nativeTerminalHost.includes('if (renderingPaused || imeTransitionActive)'));
    assert.ok(nativeTerminalHost.includes('if (!useNativeToolbar) {'));
    assert.ok(nativeTerminalHost.includes('imeCompensationBottomPx = 0'));
  ...
  test('cloud build derives the next Android build number from the latest release artifact', () => {
    ...
    assert.ok(workflow.includes('MINIMUM_RELEASE_BUILD=163'));
