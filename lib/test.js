...
async function runTests() {
  ...
  test('Android host exposes the native terminal pager and native browser action bridge', () => {
    ...
    assert.ok(nativeTerminalPagerActivity.includes('terminalView.requestToolbarVisible()'));
  ...
  test('saving CLI API settings synchronizes the local Codex proxy process', () => {
    ...
    assert.ok(cliApiConfig.includes('set -a; . $_codexProxyEnvPath; set +a'));
    assert.ok(cliApiConfig.includes('nohup python3 $_codexProxyPath'));
    assert.ok(cliApiConfig.includes('nohup node $_codexProxyJsPath'));
    ...
    assert.ok(cliApiDialog.includes('selectedProfileId != _sharedProfileId'));
