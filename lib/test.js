...
async function runTests() {
  ...
  test('terminal screen keeps IME from resizing the platform-view layout', () => {
    ...
    assert.ok(terminalScreen.includes('TerminalToolbar.toolbarHeight + bottomSystemInset'));
    assert.ok(terminalScreen.includes('padding: EdgeInsets.only(bottom: toolbarReservedHeight)'));
    assert.ok(terminalScreen.includes('final systemPanLift = (baselineTop - viewportTop).clamp(0.0, keyboardInset);'));
    assert.ok(terminalScreen.includes('bottom: _imeToolbarLift'));
  });

  test('terminal toolbar keeps bottom safe-area padding while lifting above the keyboard', () => {
