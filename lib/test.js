...
async function runTests() {
  ...
  test('Android host exposes the native terminal pager and native browser action bridge', () => {
    ...
    assert.ok(nativeTerminalPagerActivity.includes('createActionButton("新建") { openNewSession() }'));
    ...
    assert.ok(nativeTerminalPagerActivity.includes('nativeCardDrawable(fillColor = color, strokeColor = color, radiusDp = 0)'));
    ...
    assert.ok(nativeBrowserView.includes('左右切换 Codex 自动化流程与传统网站脚本'));
