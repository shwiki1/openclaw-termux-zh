...
async function runTests() {
  ...
  test('CLI tools launch the terminal in a native Android activity', () => {
    ...
    assert.ok(cliToolsScreen.includes('NativeBridge.openNativeTerminalActivity('));
    assert.ok(cliToolsScreen.includes('TerminalService.getProotShellConfig()'));
    assert.ok(cliToolsScreen.includes('TerminalService.replaceLoginShell(args, command)'));
    assert.ok(cliToolsScreen.includes('useNativeToolbar: true'));
    assert.ok(cliToolsScreen.includes('keepAlive: true'));
  ...
  test('Android host can launch a dedicated native terminal activity and still expose browser soft-input control', () => {
    const nativeBridge = readFileSync(
      join(projectRoot, 'flutter_app/lib/services/native_bridge.dart'),
      'utf8',
    );
  ...
  test('native terminal exposes a bottom input strip for IME panning', () => {
    ...
    assert.ok(nativeTerminalView.includes('if (config.useNativeToolbar) {'));
