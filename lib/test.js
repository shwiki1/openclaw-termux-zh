...
async function runTests() {
  ...
  test('terminal screen lets the window resize so the native terminal receives the IME inset', () => {
    const terminalScreen = readFileSync(
      join(projectRoot, 'flutter_app/lib/screens/terminal_screen.dart'),
      'utf8',
    );
    ...
    assert.ok(terminalScreen.includes('resizeToAvoidBottomInset: true'));
    ...
    assert.ok(nativeBridge.includes("return 'adjustResize';"));
  ...
  test('Android native terminal can host the shortcut bar inside the platform view', () => {
    ...
    assert.ok(nativeTerminalHost.includes('private val terminalView = DeferredResizeTerminalView(context)'));
    assert.ok(nativeTerminalHost.includes('private class DeferredResizeTerminalView('));
    assert.ok(nativeTerminalHost.includes('override fun updateSize()'));
    assert.ok(nativeTerminalHost.includes('private const val IME_RESIZE_SETTLE_MS = 120L'));
  ...
  test('Android host can switch terminal and browser soft input modes', () => {
    const nativeBridge = readFileSync(
      join(projectRoot, 'flutter_app/lib/services/native_bridge.dart'),
      'utf8',
    );
  ...
  test('native terminal exposes a bottom input strip for IME panning', () => {
    ...
    assert.ok(nativeTerminalView.includes('if (useNativeToolbar) {'));
    assert.ok(nativeTerminalView.includes('terminalView.requestRectangleOnScreen(inputStripRect, true)'));
    assert.ok(nativeTerminalView.includes('container.requestRectangleOnScreen(parentInputStripRect, true)'));
    assert.ok(!nativeTerminalView.includes('requestToolbarStripVisible('));
