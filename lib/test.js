...
async function runTests() {
  ...
  test('settings opens bundled open source license page', () => {
    const settingsScreen = readFileSync(
      join(projectRoot, 'flutter_app/lib/screens/settings_screen.dart'),
      'utf8',
    );
    const licenseScreen = readFileSync(
    ...
    assert.ok(!settingsScreen.includes('_showOpenSourceLicenseDialog'));
    assert.ok(licenseScreen.includes('OpenSourceLicenseService'));
    assert.ok(licenseScreen.includes('loadRepositoryIndex'));
    assert.ok(licenseScreen.includes('loadOpenSourceNotices'));
    assert.ok(licenseScreen.includes('addPostFrameCallback'));
