...
async function runTests() {
  ...
  test('Android host can switch terminal and browser soft input modes', () => {
    ...
    assert.ok(terminalScreen.includes('with WidgetsBindingObserver'));
    assert.ok(terminalScreen.includes('didChangeMetrics()'));
    assert.ok(terminalScreen.includes('_pauseTerminalDuringImeResize()'));
    assert.ok(terminalScreen.includes('_imeResizePauseActive'));
  });
