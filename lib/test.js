...
async function runTests() {
  ...
  test('native terminal avoids duplicate IME show requests while preserving native focus handling', () => {
    const nativeTerminalWidget = readFileSync(
      join(projectRoot, 'flutter_app/lib/widgets/native_terminal_view.dart'),
      'utf8',
    );
    ...
    assert.ok(nativeTerminalHost.includes('!imm.isActive(terminalView)'));
    assert.ok(nativeTerminalHost.includes('now - lastKeyboardShowRequestElapsedMs >= 120L'));
  });
