...
async function runTests() {
  ...
  test('dashboard is reduced to CLI tools and terminal only', () => {
    const dashboardScreen = readFileSync(
      join(projectRoot, 'flutter_app/lib/screens/dashboard_screen.dart'),
      'utf8',
    );
    assert.ok(dashboardScreen.includes('CliToolsScreen'));
    assert.ok(dashboardScreen.includes('TerminalScreen'));
    assert.ok(dashboardScreen.includes('SettingsScreen'));
    assert.ok(dashboardScreen.includes('Icons.settings_outlined'));
    assert.ok(dashboardScreen.includes('dashboardCliToolsTitle'));
    assert.ok(dashboardScreen.includes('dashboardTerminalTitle'));
    for (const removedToken of [
      ...
      'BackupManagerScreen',
      'NodeScreen',
      'CommandShortcutsScreen',
      'UpdateService',
      'UpdateFlowService',
      'NodeProvider',
    ]) {
      assert.ok(!dashboardScreen.includes(removedToken), `${removedToken} should not be on dashboard`);
    }
  });

  test('native app surface is reduced to CLI tools and terminal support code', () => {
    ...
    const installStatusFormatter = readFileSync(
      join(projectRoot, 'flutter_app/lib/services/install_status_message_formatter.dart'),
      'utf8',
    );
    const removedPaths = [
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/GatewayService.kt',
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/GatewayLogPersistence.kt',
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/NodeForegroundService.kt',
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/CpolarForegroundService.kt',
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/LocalModelForegroundService.kt',
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/SshForegroundService.kt',
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/ScreenCaptureService.kt',
      'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/MediaToolbox.kt',
      'flutter_app/lib/services/dashboard_url_resolver.dart',
      'flutter_app/assets/sample_configs/openclaw',
      'flutter_app/android/app/src/main/res/xml/usb_device_filter.xml',
    ];
    const removedTokens = [
      'GatewayService',
      'GatewayLogPersistence',
      'NodeForegroundService',
      'CpolarForegroundService',
    ...
    for (const relativePath of removedPaths) {
      assert.ok(!existsSync(join(projectRoot, relativePath)), `${relativePath} should be removed`);
    }
    for (const token of removedTokens) {
      assert.ok(!manifest.includes(token), `${token} should not be declared in AndroidManifest.xml`);
      assert.ok(!nativeBridge.includes(token), `${token} should not be exposed by NativeBridge`);
      assert.ok(!mainActivity.includes(token), `${token} should not be handled by MainActivity`);
    }
    for (const permission of [
      ...
      assert.ok(!manifest.includes(permission), `${permission} should not remain in the manifest`);
    }
    for (const permission of [
      'READ_SMS',
      'RECEIVE_SMS',
      ...
      assert.ok(
        manifest.includes(`android.permission.${permission}" tools:node="remove"`),
        `${permission} should be explicitly removed during manifest merge`,
      );
      assert.ok(
        !manifest.includes(`android.permission.${permission}" />`),
        `${permission} should not be declared as an active permission`,
      );
    }
    for (const dependency of [
      'com.arthenica:ffmpeg-kit-full',
    ]) {
      assert.ok(!buildGradle.includes(dependency), `${dependency} should not be packaged`);
    }
    assert.ok(existsSync(join(projectRoot, 'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot/FloatingFileManagerService.kt')));
    assert.ok(manifest.includes('android.permission.SYSTEM_ALERT_WINDOW'));
    assert.ok(manifest.includes('.FloatingFileManagerService'));
    assert.ok(manifest.includes('androidx.core.content.FileProvider'));
    assert.ok(manifest.includes('@xml/file_paths'));
    assert.ok(existsSync(join(projectRoot, 'flutter_app/android/app/src/main/res/xml/file_paths.xml')));
    assert.ok(buildGradle.includes('androidx.recyclerview:recyclerview'));
    assert.ok(buildGradle.includes('androidx.media3:media3-exoplayer'));
    assert.ok(buildGradle.includes('androidx.media3:media3-ui'));
    assert.ok(nativeBridge.includes('requestBatteryOptimization'));
    assert.ok(nativeBridge.includes('requestStoragePermission'));
    assert.ok(nativeBridge.includes('requestOverlayPermission'));
    assert.ok(nativeBridge.includes('startFloatingFileManager'));
    assert.ok(mainActivity.includes('"requestBatteryOptimization" ->'));
    assert.ok(mainActivity.includes('"requestStoragePermission" ->'));
    assert.ok(mainActivity.includes('"requestOverlayPermission" ->'));
    assert.ok(mainActivity.includes('"startFloatingFileManager" ->'));
    assert.ok(!pubspec.includes('assets/sample_configs/openclaw/'));
    for (const token of [
      ...
      assert.ok(!installStatusFormatter.includes(token), `${token} should not remain in setup status formatting`);
    }
    assert.ok(nativeBridge.includes('startLocalApiProxy'));
  ...
  test('native lucide drawable set contains only referenced icons', () => {
    const kotlinDir = join(projectRoot, 'flutter_app/android/app/src/main/kotlin/com/nxg/openclawproot');
    const drawableDir = join(projectRoot, 'flutter_app/android/app/src/main/res/drawable-nodpi');
    const referenced = new Set();
    for (const fileName of readdirSync(kotlinDir)) {
      if (!fileName.endsWith('.kt')) continue;
      const source = readFileSync(join(kotlinDir, fileName), 'utf8');
      for (const match of source.matchAll(/R\.drawable\.(lucide_[A-Za-z0-9_]+)/g)) {
        referenced.add(match[1]);
      }
    }
    const packaged = readdirSync(drawableDir)
      .filter((fileName) => fileName.startsWith('lucide_') && fileName.endsWith('.png'))
      .map((fileName) => fileName.replace(/\.png$/, ''));
    for (const icon of packaged) {
      assert.ok(referenced.has(icon), `${icon}.png should be referenced by native Kotlin code`);
    }
    for (const icon of referenced) {
      assert.ok(packaged.includes(icon), `${icon}.png should be packaged`);
    }
  });
  ...
  test('saving CLI API settings synchronizes the local 9999 relay', () => {
    ...
    assert.ok(cliApiConfig.includes('final usesLocalProxy = _isLocalApiProxyProfile(toolConfig);'));
    assert.ok(cliApiConfig.includes('? _providerIdForLocalProxyMapping'));
    assert.ok(cliApiConfig.includes('static String? _providerIdForLocalProxyMapping('));
    ...
    assert.ok(dashboardScreen.includes('const CliToolsScreen()'));
    assert.ok(dashboardScreen.includes('const TerminalScreen()'));
  });
  ...
  test('prebuilt rootfs cleanup preserves package copyright files', () => {
    const rootfsScript = readFileSync(
      join(projectRoot, 'scripts/build-prebuilt-rootfs.sh'),
      'utf8',
    );
    assert.ok(
      rootfsScript.includes(
        'find "$ROOTFS_DIR/usr/share/doc" \\( -type f -o -type l \\) ! -name copyright -delete',
      ),
    );
    assert.ok(!rootfsScript.includes('"$ROOTFS_DIR/usr/share/doc/"*'));
  });

  test('bundled open source license assets remain packaged', () => {
    const pubspec = readFileSync(join(projectRoot, 'flutter_app/pubspec.yaml'), 'utf8');
    const settingsScreen = readFileSync(
      join(projectRoot, 'flutter_app/lib/screens/settings_screen.dart'),
      'utf8',
    );
    const openSourceLicensesScreen = readFileSync(
      join(projectRoot, 'flutter_app/lib/screens/open_source_licenses_screen.dart'),
      'utf8',
    );
    const openSourceLicenseService = readFileSync(
      join(projectRoot, 'flutter_app/lib/services/open_source_license_service.dart'),
      'utf8',
    );
    assert.ok(settingsScreen.includes('OpenSourceLicensesScreen'));
    assert.ok(openSourceLicensesScreen.includes('loadRepositoryIndex'));
    assert.ok(openSourceLicensesScreen.includes('loadOpenSourceNotices'));
    assert.ok(!openSourceLicenseService.includes('LicenseRegistry.licenses'));
    assert.ok(openSourceLicenseService.includes('OPEN_SOURCE_REPOSITORIES.md'));
    assert.ok(openSourceLicenseService.includes('OPEN_SOURCE_NOTICES.md'));
    assert.ok(openSourceLicenseService.includes('THIRD_PARTY_NOTICES.md'));
    assert.ok(openSourceLicenseService.includes('OPEN_SOURCE_SOURCES.md'));
    assert.ok(openSourceLicensesScreen.includes('settingsOpenSourceLicensesRepositoryIndex'));
    assert.ok(openSourceLicensesScreen.includes('settingsOpenSourceLicensesNotices'));
    const repositoryIndex = readFileSync(
      join(projectRoot, 'flutter_app/assets/open_source/OPEN_SOURCE_REPOSITORIES.md'),
      'utf8',
    );
    for (const expectedRepository of [
      'https://github.com/proot-me/proot',
      'https://talloc.samba.org/',
      'https://github.com/termux/libandroid-shmem',
      'https://github.com/androidx/media/tree/release/libraries/exoplayer',
      ...
      assert.ok(repositoryIndex.includes(expectedRepository));
    }
    assert.ok(pubspec.includes('assets/open_source/OPEN_SOURCE_REPOSITORIES.md'));
    assert.ok(pubspec.includes('assets/open_source/OPEN_SOURCE_NOTICES.md'));
    assert.ok(pubspec.includes('assets/open_source/THIRD_PARTY_NOTICES.md'));
    assert.ok(pubspec.includes('assets/open_source/OPEN_SOURCE_SOURCES.md'));
  });
  ...
  test('runtime setup no longer requires bundled OpenClaw', () => {
    ...
    const buildRootfs = readFileSync(join(projectRoot, 'scripts/build-prebuilt-rootfs.sh'), 'utf8');
    const metadata = readFileSync(join(projectRoot, 'scripts/prebuilt-rootfs-metadata.sh'), 'utf8');
    const fetchRootfs = readFileSync(join(projectRoot, 'scripts/fetch-prebuilt-rootfs-asset.sh'), 'utf8');
    const workflow = readFileSync(
      join(projectRoot, '.github/workflows/flutter-build.yml'),
      'utf8',
    );
    ...
    const appSource = readFileSync(join(projectRoot, 'flutter_app/lib/app.dart'), 'utf8');
    const cliToolsScreen = readFileSync(
      join(projectRoot, 'flutter_app/lib/screens/cli_tools_screen.dart'),
      'utf8',
    );
    const removedOpenClawFiles = [
      'flutter_app/lib/screens/onboarding_screen.dart',
      'flutter_app/lib/screens/providers_screen.dart',
      'flutter_app/lib/screens/provider_detail_screen.dart',
      'flutter_app/lib/screens/custom_provider_detail_screen.dart',
      'flutter_app/lib/screens/config_editor_screen.dart',
      'flutter_app/lib/screens/configure_screen.dart',
      'flutter_app/lib/screens/message_platforms_screen.dart',
      'flutter_app/lib/screens/message_platform_detail_screen.dart',
      'flutter_app/lib/screens/logs_screen.dart',
      'flutter_app/lib/widgets/gateway_controls.dart',
      'flutter_app/lib/providers/gateway_provider.dart',
      'flutter_app/lib/services/gateway_service.dart',
      'flutter_app/lib/services/provider_config_service.dart',
      'flutter_app/lib/services/message_platform_config_service.dart',
      'flutter_app/lib/services/bundled_sample_config_service.dart',
      'flutter_app/lib/services/gateway_auth_config_service.dart',
      'flutter_app/lib/models/ai_provider.dart',
      'flutter_app/lib/models/gateway_state.dart',
      'flutter_app/lib/models/message_platform.dart',
      'flutter_app/lib/screens/backup_manager_screen.dart',
      'flutter_app/lib/screens/local_model_screen.dart',
      'flutter_app/lib/screens/local_model_library_screen.dart',
      'flutter_app/lib/screens/local_model_chat_settings_screen.dart',
      'flutter_app/lib/screens/packages_screen.dart',
      'flutter_app/lib/screens/ssh_screen.dart',
      'flutter_app/lib/screens/node_screen.dart',
      'flutter_app/lib/screens/command_shortcuts_screen.dart',
      'flutter_app/lib/services/backup_service.dart',
      'flutter_app/lib/services/local_model_service.dart',
    ...
    assert.ok(bootstrapManager.includes('"runtimeReady"'));
    assert.ok(bootstrapManager.includes('"openclawInstalled" to false'));
    assert.ok(!bootstrapManager.includes('&& nodeExists && openclawExists'));
    assert.ok(!bootstrapService.includes('readInstalledVersion'));
    assert.ok(!bootstrapService.includes('installVersion('));
    assert.ok(!bootstrapService.includes('SetupStep.installingOpenClaw'));
    assert.ok(setupWizard.includes('RuntimeInstallOptions('));
    assert.ok(!setupWizard.includes('OpenClawInstallOptions'));
    assert.ok(!setupWizard.includes('installOpenClaw'));
    assert.ok(!existsSync(join(projectRoot, 'flutter_app/lib/models/openclaw_install_options.dart')));
    assert.ok(existsSync(join(projectRoot, 'flutter_app/lib/models/runtime_install_options.dart')));
    assert.ok(!setupWizard.includes('OnboardingScreen'));
    assert.ok(!setupWizard.includes('ProviderConfigService'));
    assert.ok(!setupWizard.includes('BundledSampleConfigService'));
    assert.ok(!setupWizard.includes('setupWizardConfigureApiKeys'));
    assert.ok(!setupWizard.includes('_handleConfigureApi'));
    assert.ok(!splashScreen.includes('OnboardingScreen'));
    assert.ok(!splashScreen.includes('ProviderConfigService'));
    assert.ok(!splashScreen.includes('openclaw-snapshot'));
    assert.ok(!splashScreen.includes('root/.openclaw/openclaw.json'));
    assert.ok(splashScreen.includes('CLI Runtime for Android'));
    assert.ok(!dashboardScreen.includes('MessagePlatformsScreen'));
    assert.ok(!dashboardScreen.includes("dashboardMessagePlatformsTitle"));
    assert.ok(!dashboardScreen.includes('ProvidersScreen'));
    assert.ok(!dashboardScreen.includes('dashboardProvidersTitle'));
    assert.ok(!appSource.includes('GatewayProvider'));
    assert.ok(cliToolsScreen.includes('中转代理'));
    assert.ok(cliToolsScreen.includes('API 管理'));
    assert.ok(cliToolsScreen.includes('LocalApiProxyBrowserScreen'));
    assert.ok(cliToolsScreen.includes('CliApiConfigService'));
    for (const relativePath of removedOpenClawFiles) {
      assert.ok(!existsSync(join(projectRoot, relativePath)), `${relativePath} should be removed`);
    }
    const settingsScreen = readFileSync(
      join(projectRoot, 'flutter_app/lib/screens/settings_screen.dart'),
    ...
    assert.ok(settingsScreen.includes('OpenSourceLicensesScreen'));
    assert.ok(settingsScreen.includes('settingsOpenSourceLicenses'));
    assert.ok(settingsScreen.includes('settingsBatteryOptimization'));
    assert.ok(settingsScreen.includes('settingsStorage'));
    assert.ok(settingsScreen.includes('settingsFloatingFileManager'));
    assert.ok(settingsScreen.includes('NativeBridge.requestBatteryOptimization'));
    assert.ok(settingsScreen.includes('NativeBridge.requestStoragePermission'));
    assert.ok(settingsScreen.includes('NativeBridge.startFloatingFileManager'));
    for (const removedSettingsToken of [
      'GatewayProvider',
      'NodeProvider',
      'BackupManagerScreen',
      'NodeScreen',
      ...
      assert.ok(!settingsScreen.includes(removedSettingsToken), `${removedSettingsToken} should not return to settings`);
    }
    assert.ok(!buildRootfs.includes('npm install -g --omit=dev --force "$package_spec"'));
    assert.ok(!buildRootfs.includes('openclaw plugins install'));
    assert.ok(!buildRootfs.includes('QQBOT_PACKAGE'));
    assert.ok(!buildRootfs.includes('WEIXIN_PACKAGE'));
    assert.ok(buildRootfs.includes('format=ciyuanxia-prebuilt-rootfs'));
    assert.ok(metadata.includes('"format": "ciyuanxia-prebuilt-rootfs-manifest"'));
    assert.ok(!metadata.includes('OPENCLAW_RESOLVED_VERSION'));
    assert.ok(fetchRootfs.includes('data.get("format") != "ciyuanxia-prebuilt-rootfs-manifest"'));
    assert.ok(!fetchRootfs.includes('openclaw-prebuilt-rootfs-manifest'));
    assert.ok(workflow.includes('Build CiYuanXia Apps'));
    assert.ok(workflow.includes('Build bundled CiYuanXia rootfs'));
  });
