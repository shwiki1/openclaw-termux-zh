...
async function runTests() {
  ...
  test('native terminal exposes a bottom input strip for IME panning', () => {
    ...
    assert.ok(nativeTerminalView.includes('override fun getFocusedRect(r: Rect)'));
    assert.ok(nativeTerminalView.includes('requestRectangleOnScreen(focusRect, true)'));
    assert.ok(nativeTerminalView.includes('terminalView.requestInputStripVisible()'));
  });
