...
async function runTests() {
  ...
  test('native terminal exposes a bottom input strip for IME panning', () => {
    ...
    assert.ok(nativeTerminalView.includes('private fun requestToolbarStripVisible(toolbar: HorizontalScrollView)'));
    ...
    assert.ok(nativeTerminalView.includes('container.requestRectangleOnScreen(toolbarVisibilityRect, true)'));
    assert.ok(nativeTerminalView.includes('if (useNativeToolbar) {'));
    assert.ok(nativeTerminalView.includes('toolbarStrip?.let { requestToolbarStripVisible(it) }'));
    assert.ok(nativeTerminalView.includes('private val bottomSpaceView = View(context)'));
    assert.ok(nativeTerminalView.includes('contentContainer.addView(\n            bottomSpaceView,'));
    ...
    assert.ok(nativeTerminalView.includes('val imeVisible = obscuredHeight > dpToPx(KEYBOARD_VISIBLE_THRESHOLD_DP)'));
    assert.ok(nativeTerminalView.includes('container.post(imePostLayoutRefreshRunnable)'));
